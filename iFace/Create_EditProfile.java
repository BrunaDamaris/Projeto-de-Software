...
public class Create_EditProfile {
	public static void yourprofile(ArrayList<Users> user,ArrayList<Community> systemcommunities,int idnow,Scanner input) {
		...
		if(user.contains(user.get(idnow))){
			...
			//Change User name
			System.out.println("Seu nome atual e: " + user.get(idnow).getUsername());
			...
			//Change Password
			System.out.println("Sua senha atual e: " + user.get(idnow).getPass());
			...
			//Change Status
			if(user.get(idnow).getStatus() != null) System.out.println("Seu Status atual e: " + user.get(idnow).getStatus());
			...
			//Change Address
			if(user.get(idnow).getAddress() != null) System.out.println("Seu endereco atual e: " + user.get(idnow).getAddress());
			...
			//Change Birthday
			if(user.get(idnow).getBirthday() != null) System.out.println("Sua data de nascimento atual e: "+ user.get(idnow).getBirthday());
			...
			if(ismemberofcommunity == -1) {
				...
				if(entry5.equals("1")) {
					...
					if(exitcurrentcommunityid == -1) {
						...
						String decision;
						System.out.println("Deseja sair da comunidade? Se sim ,pressione 1. Se nao, pressione enter");
			...
			System.out.println("Nao foi possivel achar sua conta");
		}
	}
}
